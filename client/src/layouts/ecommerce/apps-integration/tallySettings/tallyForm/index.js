...
const TallyFormEdit = ({ openTally, setOpenTally, alreadyConfigured, setAlreadyConfigured }) => {
  ...
  const [tallyConfigId, setTallyConfigId] = useState('');
  ...
  //fetch tally config data form api
  useEffect(() => {
    fetchTallyConfig(orgId, locId).then((res) => {
      if (res?.data?.data?.es === 1) {
        ...
        setIpAddress(res?.data?.data?.tallyConfig?.ipAddress);
        setPort(res?.data?.data?.tallyConfig?.port);
        setTallyConfigId(res?.data?.data?.tallyConfig?.tallyConfigId);
        setSyncConfigId(res?.data?.data?.tallyConfig?.syncConfig?.syncConfigId);
      }
    });
  }, []);
            ...
            <table className="sync-history-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td>{tallyConfiguredData?.tallyConfig?.port ? tallyConfiguredData?.tallyConfig?.port : '-----'}</td>
                  <td>
              ...
              <>
                ...
                <div className="form-wrapper-containerI">
                  <label htmlFor="companySelect" className="tally-label tally-advanced-label">
                    ...
                    <span className="label-description">
                      Select the Sync Date range for which you want to synchronize data with Tally. This configuration
                      will determine the settings for the chosen Sync Date range.
                    </span>
                  </label>
            ...
            {step === 8 && (
              <div className="form-wrapper-container">
                <label htmlFor="syncHistory" className="tally-label tally-advanced-label">
                  ...
                  <span className="label-description">
                    View the synchronization history for each company. This section provides details about the sync data
                    types and periods.
                  </span>
                </label>
                {tallyConfigured ? (
