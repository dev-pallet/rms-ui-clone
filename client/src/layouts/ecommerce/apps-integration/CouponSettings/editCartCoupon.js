...
const EditCartCoupon = ({ couponData }) => {
  ...
  useEffect(() => {
    const defLocations = couponData?.applicableLocation;
    const filteredLocations = locOp?.filter((location) => defLocations?.includes(location?.value));
    console.log(filteredLocations);
    setLocations(filteredLocations);
  }, [couponData?.applicableLocation, locOp]);
    ...
    createCoupon(formData)
      .then((res) => {
        if (res.data.data.es === 0) {
          setLoader(false);
          setSeverity('success');
          setErrorMsg('Coupon Updated Successfully');
          setOpen(true);
          setTimeout(() => {
