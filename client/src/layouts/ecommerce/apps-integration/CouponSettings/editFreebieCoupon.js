import './PreapprovedCoupon.css';
import * as React from 'react';
import { AdapterDayjs } from '@mui/x-date-pickers/AdapterDayjs';
import {
  Autocomplete,
  Button,
  Card,
  CircularProgress,
  Dialog,
  DialogActions,
  DialogContent,
  DialogTitle,
  Divider,
  Grid,
  InputLabel,
  TextField,
} from '@mui/material';
import { DemoContainer, DemoItem } from '@mui/x-date-pickers/internals/demo';
import { LocalizationProvider } from '@mui/x-date-pickers/LocalizationProvider';
import { TimePicker } from '@mui/x-date-pickers/TimePicker';
import {
  createCoupon,
  getAllManufacturer,
  getItemsInfo,
  getLatestInwarded,
  getLocationwarehouseData,
  getMainCategory,
  getRetailUserLocationDetails,
  getUserDetails,
  manufacturerSearch,
} from '../../../../config/Services';
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import Checkbox from '@mui/material/Checkbox';
import CloseIcon from '@mui/icons-material/Close';
import FormControl from '@mui/material/FormControl';
import FormControlLabel from '@mui/material/FormControlLabel';
import FormField from '../../purchase-bills/components/FormField';
import FormGroup from '@mui/material/FormGroup';
import MuiAlert from '@mui/material/Alert';
import Snackbar from '@mui/material/Snackbar';
import SoftBox from '../../../../components/SoftBox';
import SoftButton from '../../../../components/SoftButton';
import SoftInput from '../../../../components/SoftInput';
import SoftSelect from '../../../../components/SoftSelect';
import SoftTypography from 'components/SoftTypography';
import moment from 'moment';

const Alert = React.forwardRef(function Alert(props, ref) {
  return <MuiAlert elevation={6} ref={ref} variant="filled" {...props} />;
});

const EditFreebieCoupon = ({ couponData }) => {
  const [open, setOpen] = useState(false);
  const [severity, setSeverity] = useState('success');
  const [errormsg, setErrorMsg] = useState('');
  const [loader, setLoader] = useState(false);

  const [discount, setDiscount] = useState({ value: 'FLAT_OFF', label: 'Rs' });
  const [wholeDiscount, setwholeDiscount] = useState({ value: 'FLAT_OFF', label: 'Rs' });
  const [wholeDicountValue, setWholeDiscountValue] = useState();
  const [wholeDiscountUpto, setwholeDiscountUpto] = useState();
  const [discountType, setDiscountType] = useState({ value: 'All', label: 'All' });
  const [couponCode, setCouponCode] = useState('');
  const [offerName, setOfferName] = useState('');
  const [description, setDescription] = useState('');
  const [termsAndConditions, setTermsAndConditions] = useState('');
  const [discountValue, setDiscountValue] = useState('');
  const [discountUpto, setDiscountUpto] = useState('');
  const [limit, setLimit] = useState(false);
  const [dateFrom, setDateFrom] = useState('');
  const [dateTo, setDateTo] = useState('');
  const [timeFrom, setTimeFrom] = useState('');
  const [timeTo, setTimeTo] = useState('');
  const [totalUsage, setTotalUsage] = useState('');
  const [maxBudget, setMaxBudget] = useState('');
  const [userUsage, setUserUsage] = useState('  ');
  const [combined, setCombined] = useState(false);
  const [catOp, setCatOp] = useState([]);
  const [categories, setCategories] = useState([]);
  const [manuOp, setManuOp] = useState([]);
  const [manufacturers, setManufacturers] = useState([]);
  const [locOp, setLocOp] = useState([]);
  const [locations, setLocations] = useState([]);
  const [cartValue, setCartValue] = useState('');
  const [image, setImage] = useState('');
  const [displayImage, setDisplayImage] = useState('');
  const [checkbox, setCheckbox] = useState([]);
  const [orgIds, setOrgIds] = useState({});
  const [autoCompleteDetailsRowIndex, setAutoCompleteDetailsRowIndex] = useState([]);
  const orgId = localStorage.getItem('orgId');
  const localorgId = localStorage.getItem('orgId');
  const locId = localStorage.getItem('locId');
  const contextType = localStorage.getItem('contextType');
  const [inwardedData, setInwardedData] = useState({});
  const [selectedCategory, setSelectedCategory] = useState('All');
  const [openDialog, setOpenDialog] = useState(false);
  const [rowIndex, setRowIndex] = useState(0);
  const [couponValue, setCouponValue] = useState(0);
  const [ItemReddemed, setItemTobeRedemeed] = useState(0);
  const [standardChecked, setStandardChecked] = useState(true);
  const [comboChecked, setComboChecked] = useState(false);

  const handleStandardChange = (event) => {
    setStandardChecked(event.target.checked);
    setComboChecked(!comboChecked);
  };

  const handleComboChange = (event) => {
    setComboChecked(event.target.checked);
    setStandardChecked(!standardChecked);
  };
  const [inputlist, setInputlist] = useState([
    {
      prodOptions: [],
      itemCode: '',
      itemName: '',
      spec: '',
      quantityOrdered: 1,
      finalPrice: '',
      unit: '',
      previousPurchasePrice: 0,
      TotalAmtQty: 0,
      preferredVendor: '',
      offerType: '',
      discountType: '',
      discountValue: '',
      discountUpto: '',
      itemlabel: 'Item Name',
      quantilabel: 'Quantity',
      speclabel: 'Specification',
      pricelabel: 'Total Amount',
      vendorlabel: 'Offer Type',
    },
  ]);
  useEffect(() => {
    if (selectedCategory === 'DISCOUNT') {
      setOpenDialog(true);
    }
  }, [selectedCategory]);

  const handleCloseDialog = () => {
    setOpenDialog(false);
  };
  const handleRemove = (item, index) => {
    const newList = [...inputlist.filter((e, i) => i !== index)];
    setInputlist(newList);
    // let newSelectAuto = [];
    // for (let i = 0; i < newList.length; i++) {
    //   if (newList[i]?.itemName?.length) {
    //     newSelectAuto.push(i);
    //   }
    // }
    // setAutoCompleteDetailsRowIndex(newSelectAuto);
    // const list1 = [...productIds];
    // list1.splice(index, 1);
  };
  const selectProduct = (item, index) => {
    const list = [...inputlist];
    list[index].itemCode = item.gtin;
    list[index].itemName = item.name;
    list[index].spec = item.weights_and_measures.net_weight + ' ' + item.weights_and_measures.measurement_unit;
    list[index].unit = item.weights_and_measures.net_weight;
    list[index].finalPrice = item.mrp.mrp;
    list[index].gst = item.igst;

    setInputlist(list);
    // previPurchasePrice(item.gtin, orgId).then((res) => {
    //   list[index].previousPurchasePrice = res.data.data.previousPurchasePrice;
    // });
    setAutoCompleteDetailsRowIndex((prev) => [...prev, index]);

    const payload = {
      locationId: locId,
      gtins: [`${item.gtin}`],
    };
    getLatestInwarded(payload)
      .then((res) => {
        setInwardedData(res?.data?.data?.data[0]?.sellingPrice);
        const list = [...inputlist];
        list[index]['previousPurchasePrice'] = res?.data?.data?.data[0]?.sellingPrice;
        setInputlist(list);
      })
      .catch((err) => {});
  };

  const navigate = useNavigate();

  const handleCloseAlert = (event, reason) => {
    if (reason === 'clickaway') {
      return;
    }
    setOpen(false);
  };

  const handleNegativeVal = (event) => {
    if (event.charCode < 48 && event.charCode !== 32 && event.charCode !== 37) {
      event.preventDefault();
    }
  };

  const discounttypeop = [
    { value: 'All', label: 'All' },
    { value: 'Category', label: 'Category' },
    { value: 'Manufacturer', label: 'Manufacturer' },
  ];
  const discountop = [
    { value: 'FLAT_OFF', label: 'Rs' },
    { value: 'PERCENTAGE', label: '%' },
  ];

  const handleChange = (index) => {
    setCheckbox((prevState) => {
      return prevState.map((item, id) => {
        return index === id ? { ...item, checked: !item.checked } : item;
      });
    });
  };

  const handleClick = () => {
    setSelectedCategory('');
    setInputlist([
      ...inputlist,
      {
        prodOptions: [],
        itemCode: '',
        spec: '',
        quantityOrdered: 1,
        previousPurchasePrice: 0,
        TotalAmtQty: 0,
        unit: '',
        preferredVendor: '',
        offerType: '',
        discountType: '',
        discountValue: '',
        discountUpto: '',
        itemlabel: 'Item Name',
        speclabel: 'Specification',
        quantilabel: 'Quantity',
        pricelabel: 'Total Amount',
        vendorlabel: 'Offer Type',
      },
    ]);
  };

  const removeImage = (index) => {
    setCheckbox((prevState) => {
      return prevState.map((item, id) => {
        return index === id ? { ...item, image: '', displayImage: '' } : item;
      });
    });
  };

  const [updatedAmount, setUpdatedAmount] = useState(0);
  useEffect(() => {
    setUpdatedAmount(inwardedData * parseInt(inputlist[0]?.quantityOrdered));
  }, [inputlist]);

  useEffect(() => {
    setOfferName(couponData?.couponId);
    setCouponCode(couponData?.couponCode);
    setDescription(couponData?.description);
    setTermsAndConditions(couponData?.termsAndConditions);
    setMaxBudget(couponData?.budget);
    setLimit(couponData?.validPeriodExist);
    setDateFrom(couponData?.validFrom);
    setDateTo(couponData?.validTo);
    setTimeFrom(couponData?.validTimeFrom);
    setTimeTo(couponData?.validTimeTo);
    setTotalUsage(couponData?.maxTotalUsage);
    setCartValue(couponData?.minOrderValue);
    if (couponData?.discountType === 'FLAT_OFF') {
      setDiscount({ value: 'FLAT_OFF', label: 'Rs' });
    } else if (couponData?.discountType === 'PERCENTAGE') {
      setDiscount({
        value: 'PERCENTAGE',
        label: '%',
      });
    }
    setDiscountValue(couponData?.discountValue);
    setDiscountUpto(couponData?.discountUpto);
    setUserUsage(couponData?.maxUsagePerUser);
    setCombined(couponData?.combined);
    const checkBoxdata = couponData?.couponApplicableCustomersList?.map((item) => {
      const matchingImage = couponData?.couponImageList?.find((itemimg) => item?.customerType === itemimg?.imageType);
      const result = {
        name: item?.customerType,
        checked: true,
      };
      if (matchingImage) {
        result.displayImage = matchingImage.image;
        result.image = matchingImage.image;
      }
      return result;
    });

    setCheckbox(checkBoxdata);
  }, [couponData]);

  useEffect(() => {
    const orgId = localStorage.getItem('orgId');
    const contextType = localStorage.getItem('contextType');
    getAllManufacturer()
      .then((res) => {
        const manu = res?.data?.data.map((e) => {
          return {
            value: e.manufactureName,
            label: e.manufactureName,
          };
        });
        setManuOp(manu);
      })
      .catch((err) => {
        setSeverity('error');
        setErrorMsg('Cannot fetch manufacturers');
        setOpen(true);
      });

    getMainCategory(orgId)
      .then((res) => {
        const cat = res?.data?.data.map((e) => {
          return {
            value: e.categoryName,
            label: e.categoryName,
          };
        });
        setCatOp(cat);
      })
      .catch((err) => {
        setSeverity('error');
        setErrorMsg('Cannot fetch categories');
        setOpen(true);
      });

    if (contextType == 'RETAIL') {
      getRetailUserLocationDetails(orgId).then((res) => {
        const loc = res?.data?.data?.branches.map((e) => {
          return {
            value: e.branchId,
            label: e.displayName,
          };
        });
        setLocOp(loc);
      });
    } else if (contextType == 'WMS') {
      getLocationwarehouseData(orgId).then((res) => {
        const loc = res?.data?.data?.locationDataList.map((e) => {
          return {
            value: e.locationId,
            label: e.locationName,
          };
        });
        setLocOp(loc);
      });
    }
    getUserDetails()
      .then((res) => {
        const data = res.data.data;
        const orgId = {};
        const channels = [];
        if (
          data?.roles.includes('WAREHOUSE_USER') &&
          data?.contexts?.WAREHOUSE_USER &&
          data?.contexts?.WAREHOUSE_USER &&
          data?.contexts?.WAREHOUSE_USER?.meta?.some((item) => item?.org_id === LocalorgId)
        ) {
          channels.push({ name: 'WMS', checked: false, displayImage: '', image: '' });
          orgId.warehouseId = LocalorgId;
          orgId.warehouseLoc = locId;
        }
        if (
          data?.roles.includes('RETAIL_USER') &&
          data?.contexts?.RETAIL_USER &&
          data?.contexts?.RETAIL_USER?.meta?.some((item) => item?.org_id === LocalorgId)
        ) {
          channels.push(
            { name: 'RMS', checked: false, displayImage: '', image: '' },
            { name: 'APP', checked: false, displayImage: '', image: '' },
          );
          orgId.retailId = LocalorgId;
          orgId.retailLoc = locId;
        }
        if (
          data?.roles.includes('POS_USER') &&
          data?.contexts?.POS_USER &&
          data?.contexts?.POS_USER &&
          data?.contexts?.POS_USER?.meta?.some((item) => item?.org_id === LocalorgId)
        ) {
          channels.push({ name: 'POS', checked: false, displayImage: '', image: '' });
        }
        setCheckbox(channels);
        setOrgIds(orgId);
      })
      .catch((err) => {
        setSeverity('error');
        setErrorMsg('Cannot fetch user details');
        setOpen(true);
      });
  }, []);

  const onSelectFile = (event, index) => {
    setCheckbox((prevState) => {
      const selectedFiles = event.target.files;
      const selectedFilesArray = Array.from(selectedFiles);
      // setImage(selectedFilesArray[0]);
      const image = selectedFilesArray[0];
      // selectedFilesArray.map((e) => {
      //   setBlobImages((prev) => [...prev, e]);
      // });

      const displayImage = URL.createObjectURL(selectedFilesArray[0]);

      // setDisplayImage(imagesArray);

      // FOR BUG IN CHROME
      event.target.value = '';
      return prevState.map((item, id) => {
        return index === id ? { ...item, image: image, displayImage: displayImage } : item;
      });
    });
  };

  const convertTime = (time) => {
    const date = moment.utc(time).format('YYYY-MM-DD HH:mm:ss');
    const stillUtc = moment.utc(date).toDate();
    return moment(stillUtc).local().format('HH:mm');
  };

  const handleManuTitle = (title) => {
    if (title?.length > 2) {
      manufacturerSearch(title).then((response) => {
        const manu = response?.data?.data.map((e) => {
          return {
            value: e.manufactureName,
            label: e.manufactureName,
          };
        });
        setManuOp(manu);
      });
    }
  };
  const handleSave = () => {
    let limitExist = false;
    const customerArr = [];
    let applicableOn = '';
    checkbox.forEach(({ name, checked }) => {
      if (checked) {
        limitExist = true;
        if (name === 'POS') {
          const orgId = localStorage.getItem('orgId');
          const locId = localStorage.getItem('locId');

          applicableOn = 'END_USER';
          customerArr.push({
            orgId: orgId,
            orgLocationId: locId,
            orgType: 'RETAIL',
            customerType: name,
          });
        } else if (name === 'RMS' || name === 'APP') {
          const orgId = localStorage.getItem('orgId');
          const locId = localStorage.getItem('locId');
          applicableOn = 'ORG';
          customerArr.push({
            orgId: orgId,
            orgLocationId: locId,
            orgType: 'RETAIL',
            customerType: name,
          });
        } else {
          const orgId = localStorage.getItem('orgId');
          const locId = localStorage.getItem('locId');
          applicableOn = 'ORG';
          customerArr.push({
            orgId: orgId,
            orgLocationId: locId,
            orgType: 'WMS',
            customerType: name,
          });
        }
      }
    });
    const payloadData = inputlist?.map((e, i) => {
      return {
        itemCode: e.itemCode,
        quantity: e.quantityOrdered ? e.quantityOrdered : 1,
        specification: e.spec,
        freebieType: e.offerType,
        discountType: e.offerType === 'FREE' ? 'NONE' : e.discountType,
        discountValue: e.discountValue,
        discountUpto: e.discountUpto,
      };
    });
    const hasNonEmptyItemCode = payloadData?.some((item) => item?.itemCode !== '');
    if (
      !couponCode ||
      !offerName ||
      !maxBudget ||
      !totalUsage ||
      !locations?.length ||
      // !cartValue ||
      // (discount?.value == 'FLAT_OFF' ? !discountValue : !discountUpto || !discountValue) ||
      (limit ? !dateFrom || !dateTo : false) ||
      !customerArr.length
    ) {
      setSeverity('warning');
      setErrorMsg('Please fill all required fields');
      setOpen(true);
      return;
    } else if (totalPrice > couponValue) {
      setSeverity('error');
      setErrorMsg('Total Item Value must be less than couponValue');
      setOpen(true);
      return;
    } else if (!hasNonEmptyItemCode) {
      setSeverity('error');
      setErrorMsg('Add Freebie Product');
      setOpen(true);
      return;
    }
    const locId = localStorage.getItem('locId');
    const orgId = localStorage.getItem('orgId');
    const uidx = JSON.parse(localStorage.getItem('user_details')).uidx;
    const contextType = localStorage.getItem('contextType');

    const locArr = [];
    locations.forEach((e) => {
      locArr.push(e.value);
    });
    const manArr = [];
    manufacturers.forEach((e) => {
      manArr.push(e.value);
    });

    const catArr = [];
    categories.forEach((e) => {
      catArr.push(e.value);
    });
    if (
      customerArr?.length > 1 &&
      customerArr.some((c) => c.customerType === 'POS') &&
      customerArr.some((c) => c.customerType === 'WMS' || c.customerType === 'RMS' || c.customerType === 'APP')
    ) {
      applicableOn = 'ALL';
    }

    const payload = {
      couponId: couponData?.couponId,
      couponCode: couponCode,
      couponType: 'FREEBIE',
      createdBy: uidx,
      offerName: offerName,
      description: description,
      termsAndConditions: termsAndConditions,

      sourceLocationId: locId,
      sourceOrgType: contextType,
      sourceOrgId: orgId,
      validPeriodExist: limit,
      validFrom: dateFrom || null,
      validTo: dateTo || null,
      validTimeFrom: timeFrom || '00:00',
      validTimeTo: timeTo || '23:59',
      minOrderValue: cartValue,
      maxUsagePerUser: userUsage ? userUsage : 1,
      usageLimitExist: userUsage ? true : false,
      maxTotalUsage: totalUsage,
      budget: maxBudget,
      validPeriodExist: limit,
      combined: combined,
      preApproved: false,
      applicableOn: applicableOn,
      applicableProductType: [],
      applicableBrand: [],
      applicableLocation: locArr,
      couponApplicableCustomersList: customerArr,
      freebieSelect: 1,
      freebieItemList: payloadData,
    };
    if (comboChecked) {
      payload.discountType = wholeDiscount?.value;
      payload.discountValue = wholeDicountValue;
      if (wholeDiscount?.value === 'PERCENTAGE') {
        payload.discountUpto = wholeDiscountUpto;
      }
    } else {
      payload.discountType = 'FLAT_OFF';
      payload.discountValue = 0;
    }

    const formData = new FormData();
    formData.append(
      'coupon',
      new Blob([JSON.stringify(payload)], {
        type: 'application/json',
      }),
    );
    // if (!image) {
    //   formData.append('file', []);
    // } else {
    //   formData.append('file', image);
    // }
    checkbox.forEach(({ name, checked, image }) => {
      if (image) {
        if (name === 'POS') {
          formData.append('pos_image', image);
        }
        if (name === 'WMS') {
          formData.append('wms_image', image);
        }
        if (name === 'RMS') {
          formData.append('rms_image', image);
        }
        if (name === 'APP') {
          formData.append('app_image', image);
        }
      }
    });

    createCoupon(formData)
      .then((res) => {
        if (res.data.data.es === 0) {
          setLoader(false);
          setSeverity('success');
          setErrorMsg('Coupon Created Successfully');
          setOpen(true);
          setTimeout(() => {
            navigate('/marketing/Coupons');
          }, 2000);
        } else {
          setLoader(false);
          setSeverity('error');
          setErrorMsg(res.data.data.message);
          setOpen(true);
        }
      })
      .catch((err) => {
        setLoader(false);
        setSeverity('error');
        if (err.message === 'Request failed with status code 400') {
          setErrorMsg('Invalid max usage per customer');
        } else {
          setErrorMsg(err.message);
        }
        setOpen(true);
      });
  };

  const handleChangeIO = (e, index) => {
    const searchText = e.target.value;
    const isNumber = !isNaN(+searchText);
    const filterObject = {
      page: 1,
      pageSize: '100',
      supportedStore: [locId],
    };
    // if (contextType === 'RETAIL') {
    //   filterObject.supportedStore = ['TWINLEAVES', orgId];
    // } else if (contextType === 'WMS') {
    //   filterObject.supportedWarehouse = ['TWINLEAVES', orgId];
    // }
    if (isNumber) {
      filterObject.gtin = [searchText];
    } else {
      filterObject.names = [searchText];
    }
    if (searchText?.length >= 3) {
      //   setLoader(true);
      getItemsInfo(filterObject).then(function (response) {
        // setLoader(false);
        // console.log(response?.data?.data?.products[0].case_configuration.gtin)
        const list = [...inputlist];
        list[index].prodOptions = response?.data?.data?.products;
        setInputlist(list);
      });
    } else if (searchText == 0) {
      const list = [...inputlist];
      list[index].prodOptions = [];
      setInputlist(list);
    }
  };

  const handleChangeInput = (e, index) => {
    const { name, value } = e.target;
    if (name === 'quantityOrdered') {
      const list = [...inputlist];
      list[index][name] = value;

      const prevPrice = list[index]['previousPurchasePrice'];
      list[index]['TotalAmtQty'] = parseInt(prevPrice) * parseInt(value ? value : 1);
      setInputlist(list);
      return;
    }
    const list = [...inputlist];
    list[index][name] = value;
    setInputlist(list);
  };

  const optiondata = [
    { value: 'FREE', label: 'FREE' },
    { value: 'DISCOUNT', label: 'DISCOUNT' },
  ];
  const onCategoryChange = (event, index) => {
    setRowIndex(index);
    const list = [...inputlist];
    list[index]['offerType'] = event.value;
    setInputlist(list);
    setSelectedCategory(event.value);
  };

  const onDiscountType = (event, index) => {
    setDiscountType(event.value);
    const list = [...inputlist];
    list[index]['discountType'] = event.value;
    setInputlist(list);
  };

  const onDiscountValue = (event, index) => {
    setDiscountValue(event.target.value);
    const list = [...inputlist];
    list[index]['discountValue'] = event.target.value;
    setInputlist(list);
  };
  const onDiscountUpto = (event, index) => {
    const newValue = parseFloat(event.target.value);
    if (isNaN(newValue) || newValue <= 100) {
      setDiscountUpto(newValue);
      const list = [...inputlist];
      list[index]['discountUpto'] = newValue;
      setInputlist(list);
    }
  };

  let totalPrice = 0;

  inputlist.forEach((item) => {
    if (!isNaN(item.previousPurchasePrice)) {
      totalPrice += parseInt(item.TotalAmtQty ? item.TotalAmtQty : item.previousPurchasePrice);
    }
  });

  useEffect(() => {
    if (totalPrice > couponValue) {
      setSeverity('error');
      setErrorMsg('Total Item Value must be less than couponValue');
      setOpen(true);
    }
  }, [totalPrice]);

  return (
    <>
      <Card style={{ padding: '20px', display: 'flex' }}>
        <SoftBox style={{ display: 'flex', justifyContent: 'space-between' }}>
          <SoftTypography variant="h5" m={0.5} mb={2}>
            Edit Freebie Coupon
          </SoftTypography>
        </SoftBox>
        <div style={{ display: 'flex', margin: '15px' }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={standardChecked}
                onChange={handleStandardChange}
                name="standardCheckbox"
                color="primary"
              />
            }
            label="STANDARD"
          />
          <FormControlLabel
            style={{ marginLeft: '8px' }}
            control={
              <Checkbox checked={comboChecked} onChange={handleComboChange} name="comboCheckbox" color="primary" />
            }
            label="COMBO"
          />
        </div>
        <Grid container spacing={3}>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Coupon Title
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                value={offerName}
                onChange={(e) => setOfferName(e.target.value)}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Coupon Code
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                value={couponCode}
                onChange={(e) => setCouponCode(e.target.value)}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>Description</InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                value={description}
                placeholder="Enter description for the coupon"
                onChange={(e) => setDescription(e.target.value)}
                multiline
                rows={5}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Terms & Conditions
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                value={termsAndConditions}
                placeholder="Enter Terms & Conditions for the coupon"
                onChange={(e) => setTermsAndConditions(e.target.value)}
                // readOnly={isGen ? true : false}
                multiline
                rows={5}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Maximum Budget
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                type="number"
                value={maxBudget}
                onChange={(e) => setMaxBudget(e.target.value)}
              />
            </SoftBox>
          </Grid>

          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              {' '}
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Total Coupons to be issued
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                value={totalUsage}
                type="number"
                onChange={(e) => setTotalUsage(e.target.value)}
              />
            </SoftBox>
          </Grid>

          {/* <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Location
              </InputLabel>
              <SoftSelect value={locations} onChange={(e) => setLocations(e)} options={locOp} isMulti />
            </SoftBox>
          </Grid> */}
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Location
              </InputLabel>

              <Autocomplete
                multiple
                options={locOp}
                onChange={(event, newValue) => {
                  setLocations(newValue);
                }}
                getOptionLabel={(option) => option.label}
                filterSelectedOptions
                renderInput={(params) => <TextField {...params} placeholder="Select" />}
              />
            </SoftBox>
          </Grid>

          {comboChecked && (
            <>
              {' '}
              <Grid
                item
                xs={wholeDiscount.value === 'PERCENTAGE' ? 4 : 6}
                md={wholeDiscount.value === 'PERCENTAGE' ? 2 : 3}
                lg={wholeDiscount.value === 'PERCENTAGE' ? 2 : 3}
              >
                <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                  {' '}
                  <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                    Discount
                  </InputLabel>
                  <SoftSelect
                    options={discountop}
                    value={wholeDiscount}
                    onChange={(e) => {
                      setwholeDiscount(e);
                      setDiscountUpto('');
                      setDiscountValue('');
                    }}
                  />
                </SoftBox>
              </Grid>
              {wholeDiscount.value === 'PERCENTAGE' && (
                <>
                  <Grid item xs={4} md={2} lg={2}>
                    <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                      {' '}
                      <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                        Percentage
                      </InputLabel>
                      <SoftInput
                        onWheel={(e) => e.target.blur()}
                        onKeyPress={(e) => handleNegativeVal(e)}
                        value={wholeDicountValue}
                        type="number"
                        icon={{
                          component: '%',
                          direction: 'right',
                        }}
                        onChange={(e) => setWholeDiscountValue(e.target.value)}
                      />
                    </SoftBox>
                  </Grid>
                  <Grid item xs={4} md={2} lg={2}>
                    <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                      {' '}
                      <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                        Discount Upto
                      </InputLabel>
                      <SoftInput
                        onWheel={(e) => e.target.blur()}
                        onKeyPress={(e) => handleNegativeVal(e)}
                        value={wholeDiscountUpto}
                        type="number"
                        icon={{
                          component: '₹',
                          direction: 'left',
                        }}
                        onChange={(e) => setwholeDiscountUpto(e.target.value)}
                      />
                    </SoftBox>
                  </Grid>
                </>
              )}
              {wholeDiscount.value === 'FLAT_OFF' && (
                <Grid item xs={6} md={3} lg={3}>
                  <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                    {' '}
                    <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                      Discount Amount
                    </InputLabel>
                    <SoftInput
                      onWheel={(e) => e.target.blur()}
                      onKeyPress={(e) => handleNegativeVal(e)}
                      value={discountValue}
                      type="number"
                      icon={{
                        component: '₹',
                        direction: 'left',
                      }}
                      onChange={(e) => setDiscountValue(e.target.value)}
                    />
                  </SoftBox>
                </Grid>
              )}
            </>
          )}

          <Grid item xs={12}></Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              {' '}
              <FormGroup sx={{ marginLeft: '17px' }}>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={limit}
                      onChange={() => {
                        setLimit(!limit);
                        setDateFrom('');
                        setDateTo('');
                        setTimeFrom('');
                        setTimeTo('');
                      }}
                    />
                  }
                  label="Set Coupon Validity"
                />
              </FormGroup>
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}></Grid>
          {limit && (
            <>
              <Grid item xs={12} md={3} lg={3}>
                <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                  <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                    Valid from:
                  </InputLabel>
                  <SoftInput
                    onKeyPress={(e) => handleNegativeVal(e)}
                    onWheel={(e) => e.target.blur()}
                    type="date"
                    value={dateFrom}
                    onChange={(e) => setDateFrom(e.target.value)}
                  />
                </SoftBox>
              </Grid>

              <Grid item xs={12} md={3} lg={3}>
                <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                  <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                    Valid Upto:
                  </InputLabel>
                  <SoftInput
                    onKeyPress={(e) => handleNegativeVal(e)}
                    onWheel={(e) => e.target.blur()}
                    type="date"
                    value={dateTo}
                    onChange={(e) => setDateTo(e.target.value)}
                  />
                </SoftBox>
              </Grid>
              <Grid item xs={6} md={3} lg={3}>
                <SoftBox mb={1} ml={0.5}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DemoContainer components={['TimePicker']}>
                      <DemoItem style={{ width: '100%' }}>
                        <SoftTypography required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                          Time From:
                        </SoftTypography>
                        <TimePicker value={timeFrom} onChange={(newValue) => setTimeFrom(convertTime(newValue.$d))} />
                      </DemoItem>
                    </DemoContainer>
                  </LocalizationProvider>
                </SoftBox>
              </Grid>

              <Grid item xs={6} md={3} lg={3}>
                <SoftBox mb={1} ml={0.5}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DemoContainer components={['TimePicker']}>
                      <DemoItem style={{ width: '100%' }}>
                        <SoftTypography required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                          Time Upto:
                        </SoftTypography>
                        <TimePicker value={timeTo} onChange={(newValue) => setTimeTo(convertTime(newValue.$d))} />
                      </DemoItem>
                    </DemoContainer>
                  </LocalizationProvider>
                </SoftBox>
              </Grid>
            </>
          )}
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Minimum Cart Value
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                type="number"
                value={cartValue}
                onChange={(e) => setCartValue(e.target.value)}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Coupon Value
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                type="number"
                value={couponValue}
                onChange={(e) => setCouponValue(e.target.value)}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                No of items to be redeemed
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                style={{ maxHeight: '50px' }}
                type="number"
                value={ItemReddemed}
                onChange={(e) => setItemTobeRedemeed(e.target.value)}
              />
            </SoftBox>
          </Grid>
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              {' '}
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Maximum usage per customer
              </InputLabel>
              <SoftInput
                onKeyPress={(e) => handleNegativeVal(e)}
                onWheel={(e) => e.target.blur()}
                value={userUsage}
                type="number"
                onChange={(e) => setUserUsage(e.target.value)}
              />
            </SoftBox>
          </Grid>

          <br />
          <hr />

          {/* <Grid
            item
            xs={discount.value === 'PERCENTAGE' ? 4 : 6}
            md={discount.value === 'PERCENTAGE' ? 2 : 3}
            lg={discount.value === 'PERCENTAGE' ? 2 : 3}
          >
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              {' '}
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Discount
              </InputLabel>
              <SoftSelect
                options={discountop}
                value={discount}
                onChange={(e) => {
                  setDiscount(e);
                  setDiscountUpto('');
                  setDiscountValue('');
                }}
              />
            </SoftBox>
          </Grid>
          {discount.value === 'PERCENTAGE' && (
            <>
              <Grid item xs={4} md={2} lg={2}>
                <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                  {' '}
                  <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                    Percentage
                  </InputLabel>
                  <SoftInput onKeyPress={(e) => handleNegativeVal(e)} onWheel={(e) => e.target.blur()}
                    value={discountUpto}
                    type="number"
                    icon={{
                      component: '%',
                      direction: 'right',
                    }}
                    onChange={(e) => setDiscountUpto(e.target.value)}
                  />
                </SoftBox>
              </Grid>
              <Grid item xs={4} md={2} lg={2}>
                <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                  {' '}
                  <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                    Discount Upto
                  </InputLabel>
                  <SoftInput onKeyPress={(e) => handleNegativeVal(e)} onWheel={(e) => e.target.blur()}
                    value={discountValue}
                    type="number"
                    icon={{
                      component: '₹',
                      direction: 'left',
                    }}
                    onChange={(e) => setDiscountValue(e.target.value)}
                  />
                </SoftBox>
              </Grid>
            </>
          )}
          {discount.value === 'FLAT_OFF' && (
            <Grid item xs={6} md={3} lg={3}>
              <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                {' '}
                <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Discount Amount
                </InputLabel>
                <SoftInput onKeyPress={(e) => handleNegativeVal(e)} onWheel={(e) => e.target.blur()}
                  value={discountValue}
                  type="number"
                  icon={{
                    component: '₹',
                    direction: 'left',
                  }}
                  onChange={(e) => setDiscountValue(e.target.value)}
                />
              </SoftBox>
            </Grid>
          )}
          <Grid item xs={12} md={6} lg={6}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              {' '}
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Discount type
              </InputLabel>
              <SoftSelect
                options={discounttypeop}
                value={discountType}
                onChange={(e) => {
                  setDiscountType(e);
                  setCategories([]);
                  setManufacturers([]);
                }}
              />
            </SoftBox>
          </Grid>

          {discountType.value === 'Category' && null}
            <Grid item xs={12} md={6} lg={6}>
              <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Select Products:
                </InputLabel>
                <Autocomplete
                  multiple
                  options={catOp}
                  onChange={(event, newValue) => {
                    setCategories(newValue);
                  }}
                  getOptionLabel={(option) => option.label}
                  filterSelectedOptions
                  renderInput={(params) => <TextField {...params} placeholder="Select" />}
                />
              </SoftBox>
            </Grid>

          {discountType.value === 'Manufacturer' && (
            // <Grid item xs={12} md={6} lg={6}>
            //   <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
            //     <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
            //       Select Manufacturers:
            //     </InputLabel>
            //     <SoftSelect
            //       value={manufacturers}
            //       onChange={(e) => setManufacturers(e)}
            //       options={manuOp}
            //       isMulti
            //       delimiter
            //     />
            //   </SoftBox>
            // </Grid>
            <Grid item xs={12} md={6} lg={6}>
              <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Select Manufacturers:
                </InputLabel>

                <Autocomplete
                  multiple
                  options={manuOp}
                  onChange={(event, newValue) => {
                    setManufacturers(newValue);
                  }}
                  getOptionLabel={(option) => option.label}
                  filterSelectedOptions
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      onChange={(event) => handleManuTitle(event.target.value)}
                      placeholder="Select"
                    />
                  )}
                />
              </SoftBox>
            </Grid>
          )}
          {checkbox.some((el) => el.checked) && (
            <Grid item xs={12} md={6} lg={6}>
              <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
                {' '}
                <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Maximum usage per customer
                </InputLabel>
                <SoftInput onKeyPress={(e) => handleNegativeVal(e)} onWheel={(e) => e.target.blur()} value={userUsage} type="number" onChange={(e) => setUserUsage(e.target.value)} />
              </SoftBox>
            </Grid>
          )}
          <Grid item xs={12} md={12} lg={12}>
            <SoftBox mb={1} ml={0.5} display="flex" flexDirection="column" gap={1} className="input-container">
              <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                Combine with other coupons?
              </InputLabel>
              <SoftBox style={{ display: 'flex' }}>
                <RadioGroup
                  style={{ flexDirection: 'row', marginLeft: '17px' }}
                  value={combined ? 'yes' : 'no'}
                  onChange={(e) => setCombined(e.target.value === 'yes' ? true : false)}
                >
                  <FormControlLabel value="yes" control={<Radio />} label="Yes" />
                  <FormControlLabel value="no" control={<Radio />} label="No" />
                </RadioGroup>
              </SoftBox>
            </SoftBox>
          </Grid> */}
        </Grid>
      </Card>

      <Card></Card>

      {/* new card for adding products */}
      <Card style={{ marginTop: '20px' }}>
        <Grid container spacing={2}>
          <Grid item>
            <SoftTypography style={{ fontSize: '0.95rem', marginLeft: '50px', marginTop: '20px', fontWeight: 'bold' }}>
              Freebie Item{' '}
            </SoftTypography>
          </Grid>
          <Grid item xs={12} md={4} lg={3} style={{ marginLeft: 'auto', paddingLeft: '60px' }}>
            <SoftBox
              mb={1}
              ml={0.5}
              display="flex"
              flexDirection="column"
              gap={1}
              className="input-container"
              style={{ marginRight: '30px' }}
            >
              <SoftBox style={{ display: 'flex', alignItems: 'center' }}>
                <SoftTypography style={{ fontSize: '1rem' }}>Total item value </SoftTypography>
                <SoftTypography
                  style={{
                    fontSize: '1rem',
                    fontWeight: 'bold',
                    marginLeft: '20px',
                    backgroundColor: '#eeedf2',
                    borderRadius: '7px',
                    padding: '5px',
                    paddingInline: '10px',
                    minWidth: '40px !importnat',
                  }}
                >
                  {totalPrice}
                </SoftTypography>
                {/* <InputLabel sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Total item Value
                </InputLabel> */}
              </SoftBox>

              {/* <SoftInput onKeyPress={(e) => handleNegativeVal(e)} onWheel={(e) => e.target.blur()}
                disabled
                style={{ maxHeight: '50px' }}
                type="number"
                value={totalPrice}
                onChange={(e) => setCartValue(e.target.value)}
              /> */}
            </SoftBox>
          </Grid>
        </Grid>

        <Grid container>
          <Grid item xs={12}>
            <SoftBox className="create-pi-card23">
              {/* <Box padding={0.5}>
        <SoftTypography  style={{marginLeft:"30px" , fontSize:"0.8rem", fontWeight:"600" , }}>Item Table</SoftTypography>
      </Box> */}

              <div style={{ display: 'flex', flexDirection: 'column', flexWrap: 'nowrap' }}>
                <div>
                  <SoftBox className="infinite-input-box-I-vendor">
                    {inputlist.map((x, i) => {
                      return (
                        <Grid
                          container
                          key={i}
                          className="inline-input"
                          style={{
                            display: 'flex',
                            flexWrap: 'nowrap',
                            marginBottom: i == 0 ? '20px' : '0px',
                          }}
                        >
                          <Grid item xs>
                            <SoftBox
                              className="inline-input-box1-vendor-coupon"
                              style={{ minWidth: '20px', display: 'flex', flexDirection: 'column' }}
                            >
                              <SoftTypography
                                component="label"
                                variant="caption"
                                fontWeight="bold"
                                textTransform="capitalize"
                                fontSize="13px"
                                style={{ marginLeft: '-130px', display: i == 0 ? '' : 'none', marginBottom: '5px' }}
                              >
                                {x.itemlabel}
                              </SoftTypography>
                              {autoCompleteDetailsRowIndex.includes(i) && x.itemName?.length ? (
                                <TextField value={x.itemName} readOnly={true} style={{ width: '200px' }} />
                              ) : (
                                <Autocomplete
                                  className="search-input"
                                  options={x.prodOptions}
                                  getOptionLabel={(option) => option.name}
                                  onChange={(e, v) => selectProduct(v, i)}
                                  style={{ width: '200px', marginLeft: '-200px' }}
                                  renderInput={(params) => (
                                    <TextField
                                      {...params}
                                      onChange={(e) => handleChangeIO(e, i)}
                                      variant="outlined"
                                      name="itemCode"
                                    />
                                  )}
                                />
                              )}
                            </SoftBox>
                          </Grid>

                          <Grid item xs>
                            <SoftBox
                              className="soft-corner-box-vendorI"
                              style={{
                                minWidth: '20px',
                                maxWidth: '150px',
                                display: 'flex',
                                flexDirection: 'column',
                                marginLeft: '-40px',
                              }}
                            >
                              <SoftTypography
                                component="label"
                                variant="caption"
                                fontWeight="bold"
                                textTransform="capitalize"
                                fontSize="13px"
                                style={{
                                  display: i == 0 ? '' : 'none',
                                  marginLeft: '-30px',
                                  marginRight: '15px',
                                  marginBottom: '5px',
                                }}
                              >
                                {x.speclabel}
                              </SoftTypography>
                              <SoftInput
                                onKeyPress={(e) => handleNegativeVal(e)}
                                onWheel={(e) => e.target.blur()}
                                type="text"
                                name="unit"
                                value={x.spec}
                                style={{ minWidth: '160px !important' }}
                              />
                            </SoftBox>
                          </Grid>

                          <Grid item xs>
                            <SoftBox
                              className="soft-corner-box-vendorI"
                              style={{
                                minWidth: '20px',
                                maxWidth: '160px',
                                display: 'flex',
                                flexDirection: 'column',
                              }}
                            >
                              <SoftTypography
                                component="label"
                                variant="caption"
                                fontWeight="bold"
                                textTransform="capitalize"
                                fontSize="13px"
                                style={{ display: i == 0 ? '' : 'none', marginLeft: '-60px', marginBottom: '5px' }}
                              >
                                {x.quantilabel}
                              </SoftTypography>
                              <SoftInput
                                onKeyPress={(e) => handleNegativeVal(e)}
                                onWheel={(e) => e.target.blur()}
                                type="number"
                                name="quantityOrdered"
                                value={x?.quantityOrdered}
                                onChange={(e) => handleChangeInput(e, i)}
                              />
                            </SoftBox>
                          </Grid>

                          <Grid item xs>
                            <SoftBox
                              className="soft-corner-box-vendorI"
                              style={{
                                minWidth: '20px',
                                maxWidth: '100px',
                                display: 'flex',
                                flexDirection: 'column',
                              }}
                            >
                              <SoftTypography
                                component="label"
                                variant="caption"
                                fontWeight="bold"
                                textTransform="capitalize"
                                fontSize="13px"
                                style={{ display: i == 0 ? '' : 'none', marginBottom: '5px' }}
                              >
                                {x.pricelabel}
                              </SoftTypography>
                              <SoftInput
                                onKeyPress={(e) => handleNegativeVal(e)}
                                onWheel={(e) => e.target.blur()}
                                type="text"
                                name="previousPurchasePrice"
                                value={x.TotalAmtQty ? x.TotalAmtQty : x.previousPurchasePrice}
                                //  value={updatedAmount ? updatedAmount : inwardedData}
                              />
                            </SoftBox>
                          </Grid>

                          <Grid item xs style={{ justifyContent: 'space-between', gap: '2px', display: 'flex' }}>
                            <SoftBox
                              className="inline-input-box3"
                              style={{
                                minWidth: '20px',
                                maxWidth: '150px',
                                display: 'flex',
                                flexDirection: 'column',
                              }}
                            >
                              <SoftTypography component="label" variant="caption" fontSize="13px"></SoftTypography>

                              <SoftTypography
                                component="label"
                                variant="caption"
                                fontWeight="bold"
                                textTransform="capitalize"
                                fontSize="13px"
                                style={{ display: i == 0 ? '' : 'none', marginLeft: '15px', marginBottom: '5px' }}
                              >
                                {x.vendorlabel}
                              </SoftTypography>
                              <SoftBox className="soft-select-vendor-box">
                                <SoftBox style={{ maxWidth: '10rem !important' }}>
                                  <SoftSelect
                                    options={optiondata}
                                    placeholder="Category"
                                    onChange={(e) => onCategoryChange(e, i)}
                                    menuPortalTarget={document.body}
                                    style={{ maxWidth: '10rem !important' }}
                                  />
                                </SoftBox>
                              </SoftBox>
                            </SoftBox>

                            <SoftBox
                              className="close-icons"
                              style={{ marginLeft: '120px', marginTop: i == 0 ? '18px' : '9px' }}
                            >
                              <Grid item>
                                <CloseIcon onClick={() => handleRemove(x, i)} />
                              </Grid>
                            </SoftBox>
                          </Grid>

                          {/* <SoftBox className="close-icons" >
                    <Grid item >
                    <CloseIcon onClick={() => handleRemove(x, i)} />
                    </Grid>
                  </SoftBox> */}
                        </Grid>
                      );
                    })}
                  </SoftBox>
                </div>
              </div>

              <Button onClick={handleClick} className="add-button" style={{ marginBottom: '10px' }}>
                Add more
              </Button>
            </SoftBox>
          </Grid>
        </Grid>
      </Card>

      {/* new card for sales channel */}
      <Card style={{ marginTop: '20px' }}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={12} lg={12}>
            <SoftBox
              mb={1}
              ml={0.5}
              display="flex"
              flexDirection="row"
              gap={1}
              className="input-container"
              style={{ height: '100%', paddingLeft: '30px' }}
            >
              <SoftBox style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <InputLabel required sx={{ fontWeight: 'bold', fontSize: '0.75rem', color: '#344767' }}>
                  Sales Channels
                </InputLabel>
                <FormControl component="fieldset" variant="standard">
                  <FormGroup sx={{ marginLeft: '17px' }}>
                    {checkbox.map(({ name, checked, displayImage, image }, i) => (
                      <>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={checked}
                              onChange={() => {
                                handleChange(i);
                                removeImage(i);
                              }}
                              name={name}
                            />
                          }
                          label={name}
                          key={i}
                        />
                        {checked && (
                          <SoftBox className="upload-box">
                            <input
                              type="file"
                              name="images"
                              id={`file-${i}`}
                              onChange={(e) => {
                                onSelectFile(e, i);
                              }}
                              accept="image/png , image/jpeg, image/webp"
                            />
                            {image ? (
                              <SoftBox display="flex" flexDirection="column" alignItems="center" gap={2}>
                                <SoftBox
                                  component="img"
                                  src={displayImage}
                                  key={displayImage}
                                  alt=""
                                  borderRadius="lg"
                                  shadow="lg"
                                  width="100px"
                                  height="100px"
                                />
                                <p
                                  className="body-label"
                                  onClick={() => {
                                    removeImage(i);
                                  }}
                                >
                                  Remove image
                                </p>
                              </SoftBox>
                            ) : (
                              <label variant="body2" htmlFor={`file-${i}`} className="body-label">
                                Browse Image
                              </label>
                            )}
                          </SoftBox>
                        )}
                      </>
                    ))}
                  </FormGroup>
                </FormControl>
              </SoftBox>
              {/* {checkbox.some((el) => el.checked) && (
                <SoftBox className="multiple-box">
                  <input
                    type="file"
                    name="images"
                    id="file"
                    onChange={onSelectFile}
                    accept="image/png , image/jpeg, image/webp"
                  />
                  {image ? (
                    <SoftBox display="flex" flexDirection="column" alignItems="center" gap={2}>
                      <SoftBox
                        component="img"
                        src={displayImage}
                        key={displayImage}
                        alt=""
                        borderRadius="lg"
                        shadow="lg"
                        width="100px"
                        height="100px"
                      />
                      <p
                        className="body-label"
                        onClick={() => {
                          setImage('');
                          setDisplayImage('');
                        }}
                      >
                        Remove image
                      </p>
                    </SoftBox>
                  ) : (
                    <label variant="body2" htmlFor="file" className="body-label">
                      Browse Image
                    </label>
                  )}
                </SoftBox>
              )} */}
            </SoftBox>
          </Grid>
        </Grid>
      </Card>
      <SoftBox display="flex" flexDirection="row-reverse" className="input-container">
        <SoftButton variant="gradient" color="info" style={{ marginLeft: '20px', margin: '10px' }} onClick={handleSave}>
          {loader ? (
            <CircularProgress
              size={18}
              sx={{
                color: '#fff',
              }}
            />
          ) : (
            <>Save</>
          )}
        </SoftButton>
        <SoftButton
          variant="outlined"
          color="info"
          onClick={() => navigate(-1)}
          style={{ marginLeft: '20px', margin: '10px' }}
        >
          Cancel
        </SoftButton>
      </SoftBox>
      <Dialog
        open={openDialog}
        onClose={handleCloseDialog}
        sx={{
          '& .MuiDialog-container': {
            '& .MuiPaper-root': {
              width: '100%',
              maxWidth: '450px',
            },
          },
        }}
      >
        <DialogTitle style={{ fontSize: '0.8rem' }}>Discount</DialogTitle>
        <Divider sx={{ margin: 0 }} />
        <DialogContent>
          <SoftTypography style={{ fontSize: '0.8rem' }}>Discount Type</SoftTypography>
          <SoftBox style={{ marginTop: '10px' }}>
            <SoftSelect
              // onChange={onDiscountType}
              onChange={(e) => onDiscountType(e, rowIndex)}
              autoFocus
              options={discountop}
              placeholder="Select Provider"
              maxWidth="sm"
              fullWidth
              // menuPortalTarget={document.body}
            />
          </SoftBox>

          <FormField
            autoFocus
            margin="dense"
            label="Discount Value"
            type="number"
            value={inputlist[rowIndex]?.discountValue}
            onChange={(e) => onDiscountValue(e, rowIndex)}
            maxWidth="sm"
            fullWidth
            required
          />

          {discountType === 'PERCENTAGE' && (
            <FormField
              autoFocus
              margin="dense"
              label="Discount Upto"
              type="number"
              value={inputlist[rowIndex].discountUpto}
              onChange={(e) => onDiscountUpto(e, rowIndex)}
              maxWidth="sm"
              fullWidth
              required
            />
          )}
          <br />
        </DialogContent>
        <DialogActions>
          <SoftButton onClick={handleCloseDialog}>Cancel</SoftButton>
          <SoftButton color="info" onClick={handleCloseDialog}>
            Save
          </SoftButton>
        </DialogActions>
      </Dialog>
      <Snackbar open={open} autoHideDuration={3000} onClose={handleCloseAlert}>
        <Alert onClose={handleCloseAlert} severity={severity} sx={{ width: '100%' }}>
          {errormsg}
        </Alert>
      </Snackbar>
    </>
  );
};

export default EditFreebieCoupon;
