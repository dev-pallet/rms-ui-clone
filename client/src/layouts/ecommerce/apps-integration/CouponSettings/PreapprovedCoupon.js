...
const PreapprovedCoupon = () => {
  ...
  useEffect(() => {
    ...
    getUserDetails()
      .then((res) => {
        ...
        // if (data?.roles.includes('WAREHOUSE_USER') && data?.contexts?.WAREHOUSE_USER  && data?.contexts?.WAREHOUSE_USER && data?.contexts?.WAREHOUSE_USER?.meta?.some(item => item?.org_id === LocalorgId)) {
        //   channels.push({ name: 'WMS', checked: false, displayImage: '', image: '' });
        //   orgId.warehouseId = LocalorgId;
        //   orgId.warehouseLoc = locId;
        // }
        // if (data?.roles.includes('RETAIL_USER') && data?.contexts?.RETAIL_USER && data?.contexts?.RETAIL_USER?.meta?.some(item => item?.org_id === LocalorgId)) {
        //   channels.push(
        //     { name: 'RMS', checked: false, displayImage: '', image: '' },
        //     { name: 'APP', checked: false, displayImage: '', image: '' },
        //   );
        ...
        // }
        // if (data?.roles.includes('POS_USER')  && data?.contexts?.POS_USER && data?.contexts?.POS_USER && data?.contexts?.POS_USER?.meta?.some(item => item?.org_id === LocalorgId)) {
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      couponType: discountType.value === 'All' ? 'CART_VALUE' : 'PRE_APPROVED',
      createdBy: uidx,
      offerName: offerName,
...
export default PreapprovedCoupon;
