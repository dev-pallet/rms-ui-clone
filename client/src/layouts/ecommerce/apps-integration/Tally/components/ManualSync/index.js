...
const ConfigIpAndPort = ({
  ...
  const verifyPayload = (data) => {
    if (data.port.length < 4) {
      showSnackbar('Port number should be at least of 4 digits');
      return false;
    }
    return true;
  };
...
const DateRange = ({
  ...
  totalCompany,
  handleSelectCompanyToImportData,
}) => {
  return (
...
const ManualSync = () => {
  ...
  const steps = ['Ip Address & Port', 'Sync Data', 'Date Range', 'Sync Result'];
  const stepIcons = [
  ...
  return (
    <DashboardLayout>
      ...
      <SoftBox className="tally-manual-sync">
        <SoftBox className="tally-info">
          ...
          <SoftBox className="step-content-manual-sync">{getStepContent(activeStep)}</SoftBox>
        </SoftBox>
        <SoftBox className="tally-btns-manual-sync">
