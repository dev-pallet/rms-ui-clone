...
const GetOfferBatch = ({
  ...
  useEffect(() => {
    if (allAvailableUnits.length > 0) {
      ...
      //   return unit.batchNo && !offerbatchNum.includes(unit.batchNo);
      // });
  ...
  const handleCheckboxChange = (item, index) => {
    ...
    const updatebatchNumber = [...offerbatchNum];
    updatebatchNumber[count - 1] = item?.batchNo;
    setOfferBatchNum(updatebatchNumber);

    const updatedmrp = [...offermrp];
    updatedmrp[count - 1] = item?.mrp;
    setOfferMrp(updatedmrp);

    const updateSellingPrice = [...offersellingPrice];
    updateSellingPrice[count - 1] = item?.sellingPrice;
    setOfferSellingPrice(updateSellingPrice);
  };
