...
const StepItemPromotion = ({
  ...
  const handleAddmore = () => {
    ...
    setAutocompleteTitleOptions([]);
    setAutocompleteBarcodeOptions([]);
    // if (allAvailableUnits.length === 1) {
    //   setBatchNum([...batchNum, batchNum[0]]);
    //   setMrp([...mrp, mrp[0]]);
    //   setSellingPrice([...sellingPrice, sellingPrice[0]]);
    if (allAvailableUnits.length > 1) {
      //   setBatchNum([...batchNum, '']);
      //   setMrp([...mrp, '']);
      //   setSellingPrice([...sellingPrice, '']);
...
export default StepItemPromotion;
