...
const ListAllBatch = ({
  ...
  const handleTransferBatch = (e, item, index, item_availableUnits) => {
    if(Number(e.target.value) > Number(item_availableUnits)){
      showSnackbar('transfer units cannot be greater than available units', 'warning');
      return;
    }
    const result = checkBarcodeAndBatch(currGtin, item?.batchNo, barcodeNum, batchNumber);
    if (result) {
                        ...
                        <div>
                          ...
                          <SoftBox width="70px">
                            <SoftInput
                              ...
                              onChange={(e) => {
                                const item_availabelUnits = item?.availableUnits;
                                handleTransferBatch(e, item, index, item_availabelUnits);
                              }}
                            />
                          </SoftBox>
                        </div>
